...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", required=False, help="Browser Search instruction")
    parser.add_argument("--prompt-file", required=False, help="Path to text file containing prompt")
    ...
    # Read prompt from file if provided, else use --prompt argument
    prompt = None
    if args.prompt_file:
        ...
        print("Error: You must provide either --prompt or --prompt-file.")
        sys.exit(1)

    result = live_view_with_nova_act(
        prompt, args.starting_page, args.nova_act_key, args.region
    )
